...
const processor = async ({
  ...
  .update({
    ...
    process_end: new Date(),
    model_urls: [
      id + "/models/" + "model.usdz", 
      id + "/models/" + "model.obj", 
      id + "/models/" + "model.mtl", 
      id + "/models/" + "baked_mesh_ao0.png",
      id + "/models/" + "baked_mesh_norm0.png",
      id + "/models/" + "baked_mesh_tex0.png"
    ],
  }).eq("id", parseInt(id))
...
const _downloadFromSupabase = async (id, files) => {
  try {
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const supabaseLocation = `${id}/images/${file_name}`;
      const localLocation = `${localImageLocation}${file_name}`;

      // scarico il file da supabase
      console.log("Downloading", supabaseLocation);
      console.time(supabaseLocation);
      const { data: dataFiles, error: errorFiles } = await supabase.storage
        .from(BUCKET)
        .download(supabaseLocation);
      console.timeEnd(supabaseLocation);
      console.log("Downloaded", supabaseLocation);
