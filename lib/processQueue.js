...
const _downloadFromSupabase = async (file_location, files) => {
  try {
    const locationPath = `projects/${file_location}`;
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const supabaseLocation = `${file_location}/images/${file_name}`;
      const localLocation = `${localImageLocation}${file_name}`;

      // scarico il file da supabase
      console.time("download" + file_name);
      const { data: dataFiles, error: errorFiles } = await supabase.storage
        .from(BUCKET)
        .download(supabaseLocation);
      console.timeEnd("download" + file_name);

      if (errorFiles) throw errorFiles;

      const blob = dataFiles;
      const buffer = Buffer.from(await blob.arrayBuffer());

      // scrivo il file in locale
      console.time("write" + file_name);
      await fs.promises.writeFile(localLocation, buffer);
      console.timeEnd("write" + file_name);

      // elimino il file da supabase
      console.time("remove" + file_name);
      await supabase.storage.from(BUCKET).remove([supabaseLocation]);
      console.timeEnd("remove" + file_name);
    }
  } catch (error) {
    console.error(error);
  }
...
const _uploadDir = async ({ file_location, bucket_location }) => {
  ...
  for await (const { file, filename, contentType } of _files) {
    ...
    const supabaseLocation = `${bucket_location}/model/${filename}`;
    await supabase.storage
      .from(BUCKET)
      .upload(supabaseLocation + filename, await file, {
        contentType,
      });
  }
};
...
async function worker({ project }) {
    ...
    try {
      ...
      if (isTelegram) await _downloadFromTelegram(imgDir, files);
        ...
        await _downloadFromSupabase(`${id}`, files);
      }
    ...
    try {
      ...
      await supabase
        ...
        .update({
          ...
          model_urls: [id + "models/" + "model.usdz"],
        })
        .eq("id", id);
